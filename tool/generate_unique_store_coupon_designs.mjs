...
const outDir = path.join(root, 'assets', 'social', 'unique-store-coupon-style');
...
function hasNumericDiscount(description) {
  const text = String(description || '');
  return /(\d+|[٠-٩]+)\s*%/.test(text) || /(\d+)\s*(ريال|ر\.س|RS|SAR)/i.test(text);
}
...
function pageHtml(store, coupon, formatKey, logoData) {
  ...
  const variant = hashText(store.slug || store.id || store.name_ar || store.name_en) % 8;
  ...
  const numericDiscount = hasNumericDiscount(description);
  const discount = numericDiscount ? extractDiscount(description) : 'كود';
  const discountWord = 'خصم';
  const code = coupon?.code || 'وفرها صح';
  const shortDesc = trimText(description, tall ? 72 : 58);
  const brandTop = tall ? 52 : 38;
  const contentTop = tall ? 360 : 245;
  const couponBottom = tall ? 395 : 252;
...
.blob-top{position:absolute;width:${Math.round(700 * scale)}px;height:${Math.round(640 * scale)}px;border-radius:0 0 55% 55%;background:var(--primary);left:${Math.round(-170 * scale)}px;top:${Math.round(-160 * scale)}px;z-index:1}
.blob-bottom{position:absolute;width:${Math.round(590 * scale)}px;height:${Math.round(590 * scale)}px;border-radius:50%;background:var(--secondary);right:${Math.round(-220 * scale)}px;bottom:${Math.round(-220 * scale)}px;z-index:1}
...
.top-brand{position:absolute;right:${Math.round(72 * scale)}px;top:${brandTop}px;z-index:6;display:flex;direction:rtl;align-items:center;gap:${Math.round(16 * scale)}px}
.app-logo{width:${Math.round((tall ? 88 : 72) * scale)}px;height:${Math.round((tall ? 88 : 72) * scale)}px;border-radius:${Math.round(23 * scale)}px;box-shadow:0 14px 30px rgba(31,45,85,.16)}
.app-name{font-size:${Math.round((tall ? 34 : 28) * scale)}px;font-weight:800;line-height:1;color:var(--dark);text-align:right}
.app-tag{font-size:${Math.round((tall ? 19 : 16) * scale)}px;color:var(--muted);margin-top:${Math.round(5 * scale)}px;text-align:right;font-weight:700}
...
.store-card{position:absolute;left:50%;top:${contentTop}px;transform:translateX(-50%);z-index:5;width:${Math.round((tall ? 385 : 300) * scale)}px;height:${Math.round((tall ? 385 : 300) * scale)}px;background:#fff;border-radius:${Math.round(18 * scale)}px;display:grid;place-items:center;padding:${Math.round(46 * scale)}px;box-shadow:0 26px 52px rgba(31,45,85,.20)}
...
.code{height:${Math.round((tall ? 112 : 88) * scale)}px;border-radius:999px;background:var(--secondary);display:grid;place-items:center;color:#fff;font-size:${Math.round((tall ? 82 : 64) * scale)}px;font-weight:800;line-height:1;direction:ltr;letter-spacing:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding:0 ${Math.round(26 * scale)}px}
...
.accent-line{position:absolute;z-index:2;background:var(--primary);opacity:.22;border-radius:999px}
.accent-line.one{width:${Math.round(420 * scale)}px;height:${Math.round(38 * scale)}px;left:${Math.round(96 * scale)}px;top:${tall ? 270 : 190}px;transform:rotate(-12deg)}
.accent-line.two{width:${Math.round(360 * scale)}px;height:${Math.round(28 * scale)}px;right:${Math.round(120 * scale)}px;bottom:${tall ? 300 : 180}px;transform:rotate(10deg);background:var(--secondary)}
.layout-v1 .blob-top{left:auto;right:${Math.round(-210 * scale)}px;border-radius:0 0 0 70%;opacity:.95}.layout-v1 .blob-bottom{right:auto;left:${Math.round(-250 * scale)}px}.layout-v1 .waves{right:auto;left:${Math.round(70 * scale)}px;transform:scaleX(-1)}.layout-v1 .pill{left:${Math.round(305 * scale)}px}
.layout-v2 .blob-top{width:${Math.round(820 * scale)}px;height:${Math.round(420 * scale)}px;left:${Math.round(-140 * scale)}px;top:${Math.round(-90 * scale)}px;border-radius:0 0 999px 999px}.layout-v2 .blob-bottom{width:${Math.round(720 * scale)}px;height:${Math.round(360 * scale)}px;right:${Math.round(-260 * scale)}px;bottom:${Math.round(-120 * scale)}px;border-radius:999px 0 0 0}.layout-v2 .waves{top:${tall ? 530 : 360}px;opacity:.7}.layout-v2 .frame{border-radius:${Math.round(48 * scale)}px}
.layout-v3 .blob-top{width:${Math.round(520 * scale)}px;height:${Math.round(760 * scale)}px;left:${Math.round(-190 * scale)}px;top:${Math.round(-210 * scale)}px;border-radius:0 0 80% 0}.layout-v3 .blob-bottom{right:${Math.round(-150 * scale)}px;bottom:${Math.round(-270 * scale)}px}.layout-v3 .store-card{border-radius:50%}.layout-v3 .waves{right:${Math.round(35 * scale)}px;opacity:.82}
.layout-v4 .blob-top{width:${Math.round(610 * scale)}px;height:${Math.round(610 * scale)}px;left:${Math.round(-180 * scale)}px;top:${Math.round(-180 * scale)}px;border-radius:50%}.layout-v4 .blob-bottom{width:${Math.round(610 * scale)}px;height:${Math.round(610 * scale)}px}.layout-v4 .pill{background:var(--secondary);color:#fff}.layout-v4 .code{background:var(--primary);color:var(--dark)}.layout-v4 .waves{opacity:.45}
.layout-v5 .blob-top{width:${Math.round(500 * scale)}px;height:${Math.round(920 * scale)}px;left:${Math.round(-180 * scale)}px;top:${Math.round(-260 * scale)}px;border-radius:0 0 999px 0}.layout-v5 .blob-bottom{width:${Math.round(520 * scale)}px;height:${Math.round(760 * scale)}px;border-radius:999px 0 0 0}.layout-v5 .accent-line.one{opacity:.4}.layout-v5 .store-card{box-shadow:0 30px 70px rgba(31,45,85,.22)}
.layout-v6 .board,.layout-v6{background:linear-gradient(160deg,#ffffff 0%,#f8fbfd 38%,rgba(255,255,255,.92) 100%)}.layout-v6 .blob-top{width:${Math.round(920 * scale)}px;height:${Math.round(340 * scale)}px;left:${Math.round(-180 * scale)}px;top:${Math.round(-80 * scale)}px;border-radius:0 0 999px 999px}.layout-v6 .pill{top:${tall ? 205 : 145}px}.layout-v6 .waves{right:${Math.round(40 * scale)}px;top:${tall ? 600 : 405}px}
.layout-v7 .blob-top{left:${Math.round(-260 * scale)}px;top:${Math.round(-220 * scale)}px;transform:rotate(-14deg)}.layout-v7 .blob-bottom{right:${Math.round(-260 * scale)}px;bottom:${Math.round(-140 * scale)}px;transform:rotate(-12deg);border-radius:${Math.round(90 * scale)}px}.layout-v7 .waves{right:${Math.round(110 * scale)}px;opacity:.65}.layout-v7 .frame{border-width:${Math.max(3, Math.round(4 * scale))}px}
</style>
</head>
<body>
<main class="board layout-v${variant}">
  <div class="blob-top"></div>
  <div class="blob-bottom"></div>
  <div class="soft-white"></div>
  <div class="accent-line one"></div>
  <div class="accent-line two"></div>
  ...
  <section class="headline">
    ...
    <div class="discount-word">${escapeHtml(discountWord)}</div>
  </section>
